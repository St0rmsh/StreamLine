    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-10 py-8 sm:py-12">
      <div className="mb-10 flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-display font-black text-stitch-black tracking-tight">Watch History</h1>
        ...
        {history.length > 0 && (
          confirmClear ? (
            <div className="flex items-center gap-3">
              <span className="text-xs font-bold text-red-600">Clear all history?</span>
              <button onClick={() => setConfirmClear(false)} className="text-xs font-bold text-text-muted hover:text-black">Cancel</button>
              <button onClick={handleClear} className="text-xs font-bold text-white bg-red-600 px-4 py-2 rounded-xl hover:bg-red-700 transition-colors">Confirm</button>
            </div>
          ) : (
            <button
              onClick={() => setConfirmClear(true)}
              className="flex items-center gap-2 text-xs font-bold text-text-muted hover:text-red-600 transition-colors px-4 py-2 rounded-xl hover:bg-red-50"
            >
      ...
      {loading ? (
        <div className="space-y-4">
          {Array(5).fill(0).map((_, i) => (
            <div key={i} className="flex gap-4 animate-pulse">
              <div className="w-40 aspect-video bg-stitch-grey rounded-xl shrink-0" />
              <div className="flex-1 space-y-2 py-2">
                <div className="h-4 bg-stitch-grey rounded w-2/3" />
                <div className="h-3 bg-stitch-grey rounded w-1/3" />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col items-center justify-center py-32 text-center">
          ...
          <p className="text-sm text-muted mt-2 max-w-xs">Couldn't load your watch history. Try again shortly.</p>
        </div>
      ) : history.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-32 text-center">
          ...
          <p className="text-xl font-display font-black text-black">No Watch History</p>
          <p className="text-sm text-muted mt-2 max-w-xs">Videos you watch will show up here.</p>
        </div>
      ) : (
            ...
            {history.map((entry) => {
              ...
              return (
                <motion.div
                  ...
                  className="flex gap-4 group p-2 rounded-2xl hover:bg-stitch-grey transition-colors"
                >
                  <Link to={`/video/${v._id}`} className="w-40 shrink-0 relative aspect-video rounded-xl overflow-hidden bg-stitch-grey border border-border-main">
                  ...
                  <div className="flex-1 min-w-0 py-1">
                    <Link to={`/video/${v._id}`}>
                      <h3 className="font-bold text-sm text-stitch-black line-clamp-2 hover:text-brand-orange transition-colors">
                        {v.title}
                      </h3>
                    </Link>
                  ...
                  <button
                    ...
                    className="opacity-0 group-hover:opacity-100 self-start p-2 text-text-muted hover:text-red-600 transition-all"
